...
models/azure.py
---------------
Adapter for Azure AI Foundry via the OpenAI-compatible inference endpoint.
Uses the openai Python SDK with base_url pointing to the Azure resource's
/openai/v1/ path. Implements the ModelClient contract so the query loop
never needs to know which vendor it's talking to.

OpenAI tool result format differs from Anthropic:
  - Anthropic: all results bundled into one role="user" message
...
class AzureFoundryClient(ModelClient):
    ...
    def generate(
        ...
        finish_reason = resp.choices[0].finish_reason  # "stop" or "tool_calls"
